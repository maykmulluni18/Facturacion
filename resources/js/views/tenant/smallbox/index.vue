                        ...
                        <div class="row mt-2">
                            <div class="col-md-3">
                                ...
                                <el-select v-model="form.period" @change="changePeriod">
                                    <el-option key="between_months" label="Entre Fechas" value="between_months"></el-option>
                                </el-select>
                            </div>
                            <template v-if="form.period === 'between_months' || form.period === 'between_months'">
                                <div class="col-md-3">
                                    <label class="control-label">Fecha de</label>
                                    <el-date-picker v-model="form.month_start" value-format="yyyy-MM-dd" format="dd/MM/yyyy" type="date"></el-date-picker>
                                </div>
                            </template>
                            <template v-if="form.period === 'between_months'">
                                <div class="col-md-3">
                                    <label class="control-label">Fecha al</label>
                                    <el-date-picker v-model="form.month_end" value-format="yyyy-MM-dd" format="dd/MM/yyyy" type="date"></el-date-picker>
                                </div>
                            </template>
                            ...
                            <div class="col-lg-7 col-md-7 col-md-7 col-sm-12" style="margin-top:29px">
                        <span>
                            <strong>SALDO ACTUAL: S/ {{ saldo_actual }}</strong>
                        </span>
                            </div>
...
export default {
    ...
    data() {
        return {
            ...
            recordItem: {},
            warehousesDetail: [],
            form:{period:"",month_start:moment().format("YYYY-MM-DD"),month_end:moment().format("YYYY-MM-DD")},
            pagination:{},
            search:{},
    ...
    methods: {
        ...
        changePeriod() {
            if (this.form.period === 'between_months') {
                this.form.month_start = moment().format('YYYY-MM-DD'); //'2019-01';
        ...
        getRecordsByFilter(){
            ...
            let query = `from=${this.form.month_start}&to=${this.form.month_end}`;
            return this.$http
                .get(`/${this.resource}/saldoActual?${query}`)
                .then(response => {
                    console.log("response ",response.data.saldo_actual);
                    this.saldo_actual = response.data.saldo_actual
                })
                .catch(error => {})
                .then(() => {
        ...
        getRecords() {
            ...
            return this.$http
                .get(`/${this.resource}/records?${query}`)
                .then(response => {
