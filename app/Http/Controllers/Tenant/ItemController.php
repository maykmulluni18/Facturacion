...
class ItemController extends Controller
{
    ...
    public function getRecords2(Request $request)
    {
        ...
        return $records->orderBy('description')->get();
    ...
    public function findExistProductInRecipes($id){
        $resp = false;
        $exist = RecipesSubrecipe::query()->where("item_id",$id)->get();
        if(count($exist)  > 0) $resp = true;
        return $resp;
    }
    public function updateNameRecipe($id,$request){
        $recipe = RecipesSubrecipe::query()->where("item_id",$id)->get();
        $data_ = [
            "id"=>$recipe[0]['id'],
            "name" =>$request->description,
            "sale_price"=>$recipe[0]['sale_price'],
            "type_doc"=>$recipe[0]['type_doc'],
            "quantity"=>$recipe[0]['quantity'],
            "subrecipes_supplies"=>json_encode($recipe[0]['subrecipes_supplies']),
            "cif"=>$recipe[0]['cif'],
            "costs"=>$recipe[0]['costs'],
            "item_id"=>$recipe[0]['item_id'],
    ...
    // public function updateCostByGrams2($items,$new_price,$item_id,$sale_price){
    ...
    //                         }
    //                         //  = round((( ? (((float)$subrecipes_supplies[$j]->quantity) / 1000):(float)$new_price / 1000 ) * (float)$subrecipes_supplies[$j]->quantity),5);
    //                     } 
    //                 }
    ...
    public function updateCostByGrams2($new_price,$item_id,$sale_price){
                ...
                for ($i=0; $i < count($items) ; $i++) { 
                    ...
                    for ($j=0; $j < count($subrecipes_supplies) ; $j++) { 
                        if($subrecipes_supplies[$j]->type == 'supplies'){
                            ...
                            if(array_key_exists('id_subrecipe',$subrecipes_supplies[$j])){
                                ...
                                if(count($subrecipe) > 0) {
                                    $subrecipes_supplies[$j]->costs_by_grams = (float) round(((float) $this->recalculeCostByGramsSubRecipe($subrecipe[0]) * (float) $subrecipes_supplies[$j]->quantity),5) ;
                                }
                            }
                        }
                    }
                    ...
                    if($items[$i]['type_doc'] == 'recipe' ){
                        ...
                        $costs = json_decode($items[$i]['costs']);
                        $costs->costs_unit_product = floatval(round( $sum1 + $sum2,5));
                        $costs->margin_costs_soles = (float) floatval(round( (float) $items[$i]['sale_price'] - (float) $costs->costs_unit_product,5));
                        $costs->margin_costs_procentage = (float) floatval(round(( ( (float) $items[$i]['sale_price'] - (float) $costs->costs_unit_product) / (float)$items[$i]['sale_price']) * 100,5));
                    }                    
                    $data_ = [
                        "id"=>$items[$i]['id'],
                        "name" =>$items[$i]['name'],
                        ...
                        "subrecipes_supplies"=>json_encode($subrecipes_supplies),
                        "cif"=>$items[$i]['cif'],
                        "costs"=>$update_costs == true ?  json_encode($costs) : $items[$i]['costs']??"null" ,
                        "item_id"=>$items[$i]['item_id'],
                        "created_at"=>$items[$i]['created_at'],
                        "updated_at"=>date("Y-m-d H:i:s")
                    ];
    ...
    public function store(ItemRequest $request) {
        ...
        $item->save();
        if($request->model == 'Insumos') {
            $this->saveSupplies($item->id,$request);
            $this->updateCostByGrams($request->purchase_unit_price,$item->id,$request->sale_unit_price ?(float)$request->sale_unit_price :0);
            $this->updateCostByGrams2($request->purchase_unit_price,$item->id,$request->sale_unit_price ?(float)$request->sale_unit_price :0);
        }
        $recipe_ = RecipesSubrecipe::query()->where(["item_id"=>$item->id],["type_doc"=>"recipe"])->get();
        if(count($recipe_) > 0){
            // dd("tiene");
            $recipes_sub_recipes_ = null;
            $sale_price = $request->sale_unit_price ?(float)$request->sale_unit_price :0;
            $recipes_sub_recipes_ =json_decode($recipe_[0]['subrecipes_supplies']) ;
            // dd($recipes_sub_recipes_);
            // actualizamos  el costo por gramo
            $sum1 = $this->suma1($recipes_sub_recipes_);
            $sum2 = $this->suma2(json_decode($recipe_[0]['cif'])); // cif
            $costs  = null;
            if($recipe_[0]['type_doc'] == "recipe"){ // receta
                $costs = json_decode($recipe_[0]['costs']);
                $costs->costs_unit_product = floatval(round($sum1 + $sum2,5));
                $costs->margin_costs_soles = floatval(round($sale_price - $costs->costs_unit_product,5));
                $costs->margin_costs_procentage = floatval(round((($sale_price - $costs->costs_unit_product) / $sale_price) * 100,5));
            }
            ...
            $data_ = [
                ...
                "type_doc"=>$recipe_[0]['type_doc'],
                "quantity"=>$recipe_[0]['quantity'],
                "subrecipes_supplies"=>json_encode($recipes_sub_recipes_),
                "cif"=>$recipe_[0]['cif'],
                "costs"=> $costs?json_encode($costs):"null",
                "item_id"=>$recipe_[0]['item_id'],
                "created_at"=>$recipe_[0]['created_at'],
                "updated_at"=>date("Y-m-d H:i:s")
            ];
            RecipesSubrecipe::query()->updateOrCreate(['id' => $data_['id']], $data_) ;
            DB::connection('tenant')->commit();
    ...
    public function saveSupplies($item_id,$request){
        DB::connection('tenant')->beginTransaction();
        $data = [
            "id"=>null,
            "name"=>$request->description,
            "second_name"=>$request->second_name,
            "costs_unit"=>$request->purchase_unit_price ?(float)$request->purchase_unit_price :0,
            "quantity"=>0,
            "unit"=>$request->unit_type_id == "NIU"?"Unidad":($request->unit_type_id == "KGM"?"Kilogramo":""),
            "category_supplies"=>$request->category_id,
            "item_id"=>$item_id
        ];
        ...
        if($data['id'] != null){
            ...
            for ($j=0; $j < count($items) ; $j++) { 
                ...
                for ($k=0; $k < count($sub_recipe_) ; $k++) { 
                    $array = $sub_recipe_[$k];
                    if(array_key_exists("id_subrecipe",$array) && $array['id_subrecipe'] == $data['id']){
                        $exist = true;
                        $array['name'] = $request->description;
                    }
                    else if(array_key_exists("id_supplie",$array) && $array['id_supplie'] == $data['id']){
                        $exist = true;
                        $array['name'] = $request->description;
                    }
                    $sub_recipe_[$k] = $array;
    ...
    public function destroy($id)
    {
        try {
            ...
            $item->delete();
            Supplie::where('item_id',$id)->delete();
            return [
                'success' => true,
