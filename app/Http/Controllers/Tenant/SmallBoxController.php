...
use App\Http\Resources\Tenant\SmallBoxCollection;
...
use Mpdf\Mpdf;
use App\Exports\SmallBoxExport;
...
class SmallBoxController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('tenant.smallbox.index', compact('soap_company', 'configuration'));
    }
    ...
    {
        return [
            'description_movement' => 'Descripcion del Movimiento',
            'type_movement' => 'Tipo de Movimientos',
            'date_movement' => 'Fecha del Movimiento',
            'amount_movement' => 'Monto S/'
        ];
    }
    ...
    public function records(Request $request)
    {
        ...
        dd($records); */
        return new SmallBoxCollection($records->paginate(config('tenant.items_per_page')));

    }
    public function getSaldoTotal(Request $request){
        $params = $request->all();
        $ingresos = SmallBox::query()->where("type_movement","=",0)->whereBetween('created_at', [$params['from'] . " 00:00:00", $params['to'] . " 23:59:59"])->get(); // 1 quiere decir gasto y 0 ingreso
        $amount_ingresos = 0;
        for ($i=0; $i < count($ingresos) ; $i++) { 
            $amount_ingresos+=$ingresos[$i]['amount_movement'];
        }
        $gastos = SmallBox::query()->where("type_movement","=",1)->whereBetween('created_at', [$params['from'] . " 00:00:00", $params['to'] . " 23:59:59"])->get(); // 1 quiere decir gasto y 0 ingreso
        $amount_gastos =  0;
        for ($i=0; $i < count($gastos) ; $i++) { 
            $amount_gastos+=$gastos[$i]['amount_movement'];
        }
        $saldo_actual = $amount_ingresos - $amount_gastos;
        return ["saldo_actual"=>$saldo_actual];
    }
    ...
    private function getRecords($request){
        $records = SmallBox::query()->whereBetween('created_at', [$request['from'] . " 00:00:00", $request['to'] . " 23:59:59"]);
        $records->orderBy('description_movement','asc');
    ...
    public function export(Request $request)
    {
        ...
        $items = SmallBox::query();
        $extradata = [];
        $records =  $items->get();

        return (new SmallBoxExport())
            ->setExtraData($extradata)
            ->records($records)
            ->download('Reporte_Caja_Chica_'.Carbon::now().'.xlsx');
    ...
    public function storeWithData($inputs)
    {
        ...
        try {
            ...
            $this->small_box = SmallBox::query()->updateOrCreate(['id' => $inputs['id']], $data);
            DB::connection('tenant')->commit();

            return [
                'success' => true,
                'data' => [
                    'id' => $this->small_box->id,
                    'number_full' => $this->small_box->description_movement,
