<template>
    <el-dialog :title="titleDialog"
        ...
        <div class="container">
            <div class="row">
                <div class="col">
                    <label class="control-label"> Receta <span class="text-danger">*</span></label>
                    <select v-model="item_selected" class="form-control form-control-sm mb-1" >
                        <option v-for="option in records" :value="option"> 
                            <div v-if="'type_doc'in option">
                                {{ option.name + " - Cantidad Estándar " + option.quantity_current + " gr" }} 
                            </div>
                            <div v-else>
                                {{ option.name + " - Cantidad Estándar " + option.quantity_current + " gr"}}
                            </div>
                ...
                <div class="col">
                    <label class="control-label"> Cantidad U  <span class="text-danger">*</span></label>
...
export default {
    ...
    data() {
        return {
            ...
            // titleDialog: null,
            total_profit: 0,
            titleDialog: 'Registrar Receta Inventario 3',
            total_all_profit: 0,
            loading: false,
            loadingPdf: false,
            loadingXlsx: false,
    ...
    methods: {
        ...
        getRecordsRecipes(){
            ...
            return this.$http
                ...
                .then(response => {
                    let recipes = this.parseValues(response.data.data) ;
                    this.records = recipes.filter(e=> e.type_doc == 'recipe');
                })
                .catch(error => {})
                .then(() => {
        ...
        addRegister(){
            this.loading_submit = true;
            this.item_selected_amount = this.item_selected_amount * 1000
            return this.$http
                .post(`/${this.resource}`,{quantity:this.item_selected_amount,id_recipe:this.item_selected.id,item_id:this.item_selected.item_id})
                .then(response => {
                    if(response.data.success){
                        ...
                        let info_add = {name:this.item_selected.name,quantity:this.item_selected_amount / 1000};
