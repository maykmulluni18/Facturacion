                        ...
                        <div class="row mt-1">
                            ...
                            <div class="col-lg-2">
                                <div class="form-group" :class="{'has-danger': errors.delivery_date}">
                                    <label class="control-label">Fec. Entrega <span class="text-danger"> *</span>
                            ...
                            <div class="col-lg-2">
                                <div class="form-group" :class="{'has-danger': errors.exchange_rate_sale}">
                                    <label class="control-label">Hora de Entrega <span class="text-danger"> *</span></label>
                                    <div class="d-flex">
                                        <!-- <input v-model="form.delivery_hour" type="time" id="appt" name="appt" min="09:00" max="23:59" required> -->
                                        <input v-model="form.delivery_hour" type="text" id="timeInput" >
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tr v-for="(row, index) in form.items" :key="index">
                                            ...
                                            <!-- <td>
                                                <img :src="row.image_preview" class="uploading-image" />
                                            </td> -->
...
export default {
    ...
    methods: {
            ...
            if (this.id) {
                await this.$http.get(`/${this.resource}/record/${this.id}`)
                    .then(response => {
                        ...
                        this.form.delivery_hour = (this.form.delivery_hour.split(" ")[1])
                        this.form.delivery_hour = this.form.delivery_hour.split(":00")[0]
                        this.form.delivery_hour = moment(this.form.delivery_hour, "hh:mm:ss").format('LT') 
        ...
        async clickRemoveItem(index) {
            await this.deleteImageStorage(this.form.items[index].imageurl);
            this.form.items.splice(index, 1)
            this.calculateTotal()
        },
        async deleteImageStorage(nameimage){
            await this.$http.get(`/${this.resource}/imagedelete?name=${nameimage}`)
            .then(response => {
                const data = response.data;
            })
        },
        ...
        async submit() {
            ...
            if(!this.form.delivery_date) return this.$message.error('Fec. Entrega es obligatorio');
            if(!this.form.delivery_hour) return this.$message.error('Hora de Entrega es Obligatorio');
            let timeRegex = /^(1[0-2]|0?[1-9]):([0-5][0-9])\s?(am|pm)$/i;
            let match = this.form.delivery_hour.match(timeRegex);
            if (!match) return this.$message.error('El formato de la hora de entrega es incorrecto ejemplo 12:12 am');
            let hours = match[1];
            let minutes = match[2];
            let amPm = match[3].toUpperCase();
            let time = hours + ':' + minutes + ':00' + ' ' + amPm;
            this.form.delivery_hour = moment(time, "h:mm:ss A").format("HH:mm:ss")
            this.form.delivery_hour = this.form.delivery_date + " " + this.form.delivery_hour ;
            this.loading_submit = true
            
            // await this.changePaymentMethodType(false)
            await this.$http.post(`/${this.resource}`, this.form).then(async(response) => {
