        ...
        <form autocomplete="off"
            ...
            <div class="form-body">
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <div :class="{'has-danger': errors.c}"
                             class="form-group">
                             <label class="control-label">Tematica</label>
                            <el-input v-model="form.tematica" ></el-input>
                            <small v-if="errors.tematica"
                                   class="form-control-feedback"
                                   v-text="errors.tematica[0]"></small>
                        </div>
                    </div>
                    ...
                    <div class="container">
                        <div class="row">
                            ...
                            <div class="col">
                                <div :class="{'has-danger': errors.details}"
                                    class="form-group">
                                    <label class="control-label">Detalles</label>
                                    <p><textarea v-model="form.details" name="comentario" rows="5" cols="45" placeholder="Escribe tus detalles"></textarea></p>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        async saveImage(){
            let fd = new FormData()
            fd.append('file',this.image,this.image_name)

            return this.$http.post(`/${this.resource}/image`, fd).then(response => {
        ...
        async clickAddItem() {
            ...
            let tematica =this.form.tematica || "";
            ...
            this.initForm();
            this.row.imageurl = this.image_name || "empty"
            this.row.tematica = tematica
            this.row.details = details
            if(!this.image && !this.image_name) this.$message.error('Debe Seleccionar una imagen');
